...
import Typography from '@material-ui/core/Typography';4
...
class CheckTeamAssignments extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: false,
    ...
    async componentDidUpdate(prevProps) {
        if (this.props.open !== prevProps.open) {
            ...
            await this.props.fetchSubordinateAssignments(this.props.token.accessToken);
    ...
    getUserFirstName(userId) {
        ...
        this.props.teamMembers.Children.map(user => {
            user.This.UserId == userId ? selectedUser = user.This : null;
        })
                ...
                >
                    ...
                    </Typography>
                    {
                        this.state.isLoading ? <CircularProgress /> :
                            this.props.assignments.teamAssignments.length == 0 ?
                                ...
                                <TableContainer component={Paper} className={classes.table}>
                                    <Table >
                                        ...
                                        <TableBody className={classes.tableBody}>
                                            {
                                                this.props.assignments.teamAssignments.map(assign => {
                                                    return (
                                                        <TableRow>
                                                            <TableCell>{this.getUserFirstName(assign.UserId)}</TableCell>
                                                            <TableCell>{this.getUserLastName(assign.UserId)}</TableCell>
                                                            <TableCell>{this.getUserRole(assign.UserId)}</TableCell>
...
CheckTeamAssignments.propTypes = {
...
const CheckTeamAssignmentsStyled = withStyles(styles)(CheckTeamAssignments);

export default connect(mapStateToProps, { fetchEmployeesByTopic, fetchAssignments, fetchSubordinateAssignments })(CheckTeamAssignmentsStyled);